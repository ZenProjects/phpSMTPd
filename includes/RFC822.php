...
class RFC822
{
	...
	public static function is_valid_email_address($email, $options=array())
	{
		...
		if ($options['allow_comments']){
			...
			$email = RFC822::email_strip_comments($outer_comment, $email, "(x)");
		}
		...
		if ($options['allow_comments']){
			$bits['local']	= RFC822::email_strip_comments($comment, $bits['local']);
			$bits['domain']	= RFC822::email_strip_comments($comment, $bits['domain']);
		}
